...
- canonical 音频 = `songs/<id>/song.m4a`，`/audio` 回 `audio/mp4`；**schema v3**；协议 `LOCAL_API_VERSION = 7`（N7 加了 `GET /workspaces` 与 `POST /workspaces/switch`；**以 `packages/shared/src/api-paths.ts` 为准**）。
...
- **N0b–N3 已完成**，逐批经过、判据与决策在 `PROCESS.md` 的 Phase B 段与各子计划（`docs/plans/` 下的 `…-n0.md` / `-n1.md` / `-n2.md` / `-n3.md`）。**仍然约束新代码的四条**：
  - **N1 的 D5 分段冻结**（core 的整个业务图进 `@lark/core/portable`，Metro 图 97 个模块，bundle smoke 在 `just check` 里）——**冻结文本在 N1 子计划 §8.1，那是单一事实源**。**N1 判据 22 已于 N7g 兑现**（2026-08-26，对本批新建的 dmg/tgz 跑完五套 **128/128**）。
...
- **N7 每账号独立工作区：已全部完成**（子计划 `docs/plans/2026-08-26-phase-b-mobile-n7.md`，**§8 是实施修订、§9 是 N7g 的两半**；判据 103–123 全关，测试 **3308**）。一台设备可以有多个曲库，各自绑一个账号或谁都不绑，互不可见；`local` 原地不动（零迁移），账号库在 `libraries/<32hex>/`，id = `sha256(server_id + "\n" + user_id)` 前 32 hex（与 owl 逐字节同结果）。**七条形状**：① **登录的安装必须跑在目标工作区上**（判据 116/117 逼出来的；seam 是 core 登录序列里的 `resolveTarget`）· ② **`switchWorkspace` 只写一行，绝不动 resolver 缓存**（缓存 = 「这个进程打开了哪个库」；由此 **`serving` ≠ `active`**）· ③ **门要求目标库的 `songs.db` 已在盘上** ⇒ 「先备库再翻 active」是唯一顺序 · ④ **手机 `local` 的 SecureStore 键必须不加前缀**（否则升级上来会 converge 掉自己的 outbox）· ⑤ **手机建工作区先认领 D16 身份**（intent → 库 → commit，全在 move 之前）· ⑥ **跨工作区清理 = 同一个 `runEviction` 指向别人的库**（只 SELECT ⇒ 判据 121 是构造性的），顺序先清其他工作区 · ⑦ **WAL sidecar 不能跟着搬**（迁移用 checkpoint）。**并入 = 数据库 + 音频一起复制**（用户 2026-08-26 定）。**三条新守卫进 `just check`**（`check-workspace-chokepoint.sh`）：只有 `paths.ts` 能拼 `'songs.db'` · 手机上只有 `ports/paths.ts` 能碰 `nestDirectory()` · 全仓禁「清除应用数据重来」这类现在为假的措辞。✅ **判据 122 已关**（2026-08-26）：桌面 accept 五套 **128/128**（`accept-sync` 34→36），顺带兑现 N1 判据 22 的旧账，**并抓出一个真 bug**——`lark sync login` 从不说「账号的库装在别处、要重启才打开」，于是下一条命令就说「还没有登录」（已修，见 PROCESS 的 N7g-1）。✅ **判据 123 也已关**（2026-08-26 真机会话，八步里除「账号 B 新建空库」按用户决定不测外全过；空库 / 要求重新登录 / converge 三种失败一次未现）⇒ **N7 全部完成，测试 3308**。会话带出三处跟进：① **登录现在给工作区起名**（`label`/`server_url` 此前零调用点，列表只能显示「账号曲库 085de2c3」，两端都补）· ② **切换与首次登录之后直接结束进程**（新增第五个自建原生模块 `modules/lark-app`；🔴 **`BackHandler.exitApp()` 不够**——只 finish Activity，`bootOnce` 与工作区缓存会活下来，重开等于没切）· ③ **已撤销的设备折叠**（判定在 `@lark/shared`，两端共用；它们**永远删不掉**是 skybridge 的软撤销设计，且撤销后再登录会**新注册一台**⇒ 列表只增）。**三条用户已定的不做**：账号 B 的新建空库不测 · 「登录后再问 claim/fresh」不做（手机能做桌面做不了，会两端分叉）· 彻底删除已撤销设备不做（绕不开 change 行 `device_id` 归属的协议决定，且 owl 共用同一 server）。
- **N6 收尾发布：开发中**（子计划 `docs/plans/2026-08-25-phase-b-mobile-n6.md`，五批 N6a–N6e / 判据 85–101 / 决策 a–j 全关）。用户 2026-08-25 定死范围五条：**导入 UI 与桌面一致、不加内容** · 撤销设备收尾 · **签名从简** · **只在 GitHub Release 发包、不进商店**（⇒ developer verification **不注册**）· 发版前文档大整理（到那一步再细说）；外加一条提醒 **桌面版打包记得用图标**。**N6a–N6d 已完成（判据 85–102 全关，含 2026-08-26 一次真机会话）；只剩 N6e 文档大整理 + 发版，而 N7 插在它前面**。真机那次的三条收获：**判据 86 让 N0b-3 的「出口 B」彻底作废**（2MB p95 **2ms** / 20MB **13ms**，原生比当年按纯 JS 的估算快约 500 倍）· 🔴 **判据 102 带出一条新行为**——**手机下载完一首歌会用 MMR 测得的时长覆盖 `duration`，并把它作为一条同步更新推给 workspace**（与桌面 ffprobe 差 <1ms，无害，**按现状接受只记录**）· **撤销本机之后**紧跟的列表刷新被服务器拒绝并**如实上屏**，跑一轮同步落到「登录已失效（服务器拒绝了保存的凭证）」而不是静默失败。⚠️ **手机现处于登出状态，且 LLM API Key 随卸载丢失**，要用户手动补。**N6a**（判据 85 · 87 关）：**零新依赖**——`expo-file-system@57` 自带 `File.pickFileAsync` + `file.bytes()`，子计划里的 `expo-document-picker` 作废 · **「N6's gate」已开**，`boot/runtime.ts` 现在装两件（Random + `expo-crypto` 的整文件 sha256），在此之前手机调 `parseImportFile` 会当场抛 · **判据 87 三方独立**（`shasum` 出常量 / noble 在单测复算 / expo-crypto 在设备探针复算，夹具两处共用，反测已跑）· **判据 85 成立：桌面零改动**。🔴 **两条形状要记住**：① **`ImportFileSource.read()` 是函数不是一份字节**——两阶段各读一次，URI 过期要在第二阶段如实失败；② **尺寸闸查两次**，因为 SAF `content://` 的 `size` 可能是 0，「系统没说」不等于「文件不大」。
...
│                    #   modules/lark-transfer 自建原生模块（dataSync 前台服务，N4c）
                    #   modules/lark-app 自建原生模块（quit()：结束进程，切库/首登后用，N7g）
