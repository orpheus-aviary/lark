...
🛠 **Phase B（Android，`apps/mobile`）开发中**——主计划 `docs/plans/2026-08-13-m4a-and-mobile-master-plan.md` §4（**§4.3 已于 2026-08-17 Stage-1 修订两处**）+ N0 子计划 `docs/plans/2026-08-17-phase-b-mobile-n0.md`（v4，判据 1–26 / 决策 a–l / R1–R5）。批次 N0a → N0b → N1 → …；**N0a + N0b-1 + N0b-2 + N0b-3 已完成**（`@lark/core/portable` + DatabaseContract 56 例 + `spikes/mobile-foundation/` 真机跑通契约 / bootstrap / drizzle 定案 / 卡顿·crypto·全局面，桌面测试 2480）。**D4 出口已冻结**：expo-sqlite 57.0.1 + per-call transient shim + drizzle 走 `pnpm patch`（`patches/drizzle-orm@0.38.4.patch`，未打补丁时 10k 查询漏 10000 条语句）。**N0b-3 的 release 实测**：冷启动余量两个数量级、backfill 分段 500 稳过、**apply 的生产批 500 过不去 → 暂定 200/批**（R5 定稿）、md5 端口 0.02ms 而 sha256 的阈值改绑**真实歌词尺寸**（1.94ms；256KB 上限的 86.81ms 降为标注最坏值）、**`globalThis.crypto.getRandomValues` 不存在**、**`atob` ≠ `Buffer.from(base64)`**、**`globalThis.fetch` 就是 `expo/fetch`**（三条行为全过）。下一步 **N0b-4**（播放判定 + skybridge/bilibili 探针 + 分享 intent）；**判据 19 已按用户决定改为后台 ≥5min + 蓝牙断连**。数值判据一律 release 构建。逐批状态见 `PROCESS.md` 的 Phase B 段，判据结果、版本冻结与设备档案见子计划 §9。
