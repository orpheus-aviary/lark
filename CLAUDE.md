...
- 🛠 **N4 下载：子计划已出、待开工**（`docs/plans/2026-08-20-phase-b-mobile-n4.md`，**v2，一轮反例评审收敛**，七批 N4a–N4g / 判据 40 条 / **决策 a–p 待关闭**）。用户 2026-08-20 拍板四条范围：**TLS 移出 N4**（不阻塞 N4 任何子批，**硬阻塞 N5**——主计划 §4.3 有 Stage-3 修订）· **LLM 设置页进 N4** · **收藏夹/合集批量进 N4** · **加 dataSync 前台服务**。四条开工前必须记住的：① 🔴 **明文流是头号未知**——mcdn `:8082` 那两轮成功都跑在 spike 开了 `usesCleartextTraffic` 的构建上，产品构建没有，判据 5 排在写任何 UI 之前；② 🔴 **崩溃孤儿进 `trash/recovery-*` 不是 `recovered-songs/`**（后者会被 `/sync/status` 的 `quarantined_count` 数），且清扫要 `skipSongIds: pendingFileOpSongIds(...)`；③ 🔴 **preflight 提取会把短链的 400 静默改成 502**，先补 characterization 再提取；④ **hub 与引擎同批出生**（`EngineCallbacks` 只在构造时给）。
