...
🛠 **Phase B（Android，`apps/mobile`）开发中**——主计划 `docs/plans/2026-08-13-m4a-and-mobile-master-plan.md` §4（**§4.3 已于 2026-08-17 Stage-1 修订两处**）+ N0 子计划 `docs/plans/2026-08-17-phase-b-mobile-n0.md`（v4，判据 1–26 / 决策 a–l / R1–R5）。批次 N0a → N0b → N1 → …；**N0a + N0b-1/2/3 + N0b-4a/4b 已完成**（`@lark/core/portable` + DatabaseContract 56 例 + `spikes/mobile-foundation/` 真机跑通契约 / bootstrap / drizzle 定案 / 卡顿·crypto·全局面 / bilibili 与 skybridge 探针 / 播放判定，桌面测试 2480）。**D4 出口已冻结**：expo-sqlite 57.0.1 + per-call transient shim + drizzle 走 `pnpm patch`（`patches/drizzle-orm@0.38.4.patch`，未打补丁时 10k 查询漏 10000 条语句）。**N0b-3 的 release 实测**：冷启动余量两个数量级、backfill 分段 500 稳过、**apply 的生产批 500 过不去 → 暂定 200/批**（R5 定稿）、md5 端口 0.02ms 而 sha256 的阈值改绑**真实歌词尺寸**（1.94ms；256KB 上限的 86.81ms 降为标注最坏值）、**`globalThis.crypto.getRandomValues` 不存在**、**`atob` ≠ `Buffer.from(base64)`**、**`globalThis.fetch` 就是 `expo/fetch`**（三条行为全过）。**N0b-4 的实测**：判据 22 四条硬 gate + SSE 全绿、判据 23 双网络全绿、**D17 冻结 = raw fMP4 直存达标（GO，不进任何 remux 兜底）**。下一步 **N0b-4c**（分享 intent，走 `expo-share-intent@8.0.1`）→ N0b-5（D14/D16 + GO/NO-GO 汇总）。数值判据一律 release 构建。逐批状态见 `PROCESS.md` 的 Phase B 段，判据结果、版本冻结与设备档案见子计划 §9。
