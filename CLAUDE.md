...
- **N7 每账号独立工作区：N7a–N7f 已完成，只剩 N7g**（子计划 `docs/plans/2026-08-26-phase-b-mobile-n7.md`，**§8 是实施修订、§9 是 N7g 的两半**；判据 103–122 全关，测试 **3300**）。一台设备可以有多个曲库，各自绑一个账号或谁都不绑，互不可见；`local` 原地不动（零迁移），账号库在 `libraries/<32hex>/`，id = `sha256(server_id + "\n" + user_id)` 前 32 hex（与 owl 逐字节同结果）。**七条形状**：① **登录的安装必须跑在目标工作区上**（判据 116/117 逼出来的；seam 是 core 登录序列里的 `resolveTarget`）· ② **`switchWorkspace` 只写一行，绝不动 resolver 缓存**（缓存 = 「这个进程打开了哪个库」；由此 **`serving` ≠ `active`**）· ③ **门要求目标库的 `songs.db` 已在盘上** ⇒ 「先备库再翻 active」是唯一顺序 · ④ **手机 `local` 的 SecureStore 键必须不加前缀**（否则升级上来会 converge 掉自己的 outbox）· ⑤ **手机建工作区先认领 D16 身份**（intent → 库 → commit，全在 move 之前）· ⑥ **跨工作区清理 = 同一个 `runEviction` 指向别人的库**（只 SELECT ⇒ 判据 121 是构造性的），顺序先清其他工作区 · ⑦ **WAL sidecar 不能跟着搬**（迁移用 checkpoint）。**并入 = 数据库 + 音频一起复制**（用户 2026-08-26 定）。**三条新守卫进 `just check`**（`check-workspace-chokepoint.sh`）：只有 `paths.ts` 能拼 `'songs.db'` · 手机上只有 `ports/paths.ts` 能碰 `nestDirectory()` · 全仓禁「清除应用数据重来」这类现在为假的措辞。✅ **判据 122 已关**（2026-08-26）：桌面 accept 五套 **128/128**（`accept-sync` 34→36），顺带兑现 N1 判据 22 的旧账，**并抓出一个真 bug**——`lark sync login` 从不说「账号的库装在别处、要重启才打开」，于是下一条命令就说「还没有登录」（已修，见 PROCESS 的 N7g-1）。🔴 **N7g 只剩判据 123 一次真机会话**（用户手操，清单在子计划 §9.2）。
