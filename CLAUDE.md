...
- **N5 同步开发中**（子计划 `docs/plans/2026-08-25-phase-b-mobile-n5.md`，六批 N5a–N5f / 判据 65–84 / **决策 a–j 全关**）。🔑 **2026-08-25 用户拍板改掉 D15**（主计划 §4.3 **Stage-4 修订**）：**移动端同时支持 https 与明文 http**，由设置页**一个开关**（`local_metadata.sync_allow_insecure`）决定是否接受明文——**TLS 从此不阻塞任何批次**，降为后续（负责人仍是用户）。理由是产品形状：其他用户会自建 server，而「只给某个 IP 开洞」与「支持任意自建 server」互斥（`networkSecurityConfig` 的 host 白名单是编译期 XML，运行时没有加例外的 API）。**同一次决定的第二条：接受音频走明文**——拆掉 `usesCleartextTraffic` 那道平台门之后，音频流 / 歌词平台 / 自填 LLM 端点都不再被强制 https，**判据 5 的第二半从此不证明任何东西**（注释改写，不假装还守着）。**四条形状**：① **N5 的主体不是同步逻辑**——N1f 把协调器整个搬进了 portable，宿主只填 `CoordinatorContext` 的 15 个字段，手机 **6 个现成 + 1 个常量已在 + 7 个新建**（五个在十行内）；② **触发器是唯一没有 portable 对应物的东西**（`daemon/src/sync/triggers.ts` 是 N1f 故意留下的那一半），而**后台 JS 定时器在这个项目里已经咬了四次** ⇒ **同步只在前台跑，回前台 = 一次触发且先查 token**，「后台收不到别人的改动」是如实的产品形状不是 bug；③ **首次登录必然跑全量 backfill**（`backfill.ts:63,67` 的 `done` 默认 0 / `target` 默认 1），手机从 N2 起攒的 `sync_changes` 正是 rebase 的作用对象；④ 🔴 **`imported` 行会第一次出现在手机上**——N4 §1.5 的整个简化压在「手机上没有 imported」上，而同步照拉 `file_origin`；缓存清理那一半跟着 portable 搬过来了，**危险的是 ensure-file**（imported 可能没有 `source_key` ⇒ 无处可下，要一句明确的话）。⚠️ 本机库 0 首 imported ⇒ **这条只能落单测**。**N5d-2 借了 owl 的两条流策略，两端一起改**（`portable/coordinator/stream.ts`）：**流不重放 ⇒ `onOpen` 补一轮**（服务器不发订阅之前的事件；中断后对端改动原本要等时钟，桌面 5 分钟 / 手机 15 分钟）· **流会静默 ⇒ `onFrame` 喂 60 秒看门狗**（半开 socket 一个回调都不触发，客户端永远坐在僵尸「已连接」里；**上看门狗前先核实了服务器每 25 秒真的发 ping**，没有心跳的话它会每 60 秒杀掉健康的流）。**不借 `health-probe`**（lark 的 30s 冷却 + 1s 轮询已等价）；**`trigger-gate` 的教训 lark 本来就对**（gate 问的是 session 不是凭证——owl 问错过，日志里 163 条连续 rejected）。**N5a 已完成**：`sync-labels` 四张中文表从 GUI 提取进 `@lark/shared`（characterization 先行 16 条 + 反测过，搬完一个字没改；**一处有意变化**——`SYNC_INSECURE_URL` 的文案从「请勾选下面的选项」改成「请先打开『允许明文 HTTP』」，两端共用的措辞不能描述某一端的控件位置）。
- **N4 下载：N4a–N4i 全部完成（2026-08-25，判据 1–64 全关，测试 3020）。下一站 N5 同步（**TLS 已不阻塞**，见下）/ N6 歌单导入 + 收尾发布**。**N4i 多选批量 + 行菜单补齐**（子计划 `docs/plans/2026-08-24-phase-b-mobile-n4i.md`，决策 a–j 全关）：**选择模式**（长按进入 · 全选 · **系统返回键退出**，`BackHandler` 与选择栏同生命周期 · 批量跑着时不响应返回）· 批量固定 / 取消固定 / 加入歌单 / 删除（**逐条不中断**，失败同时报两个数）· 歌单详情多一个**移出歌单**（不是删除）· 行菜单四项（添加到歌单 · 复制链接 · **用 app 打开只放行 http(s)**，`intent://` / `file://` 一律拒绝 · **更改链接**：自动识别预览 → 保存 → key 变了且有文件时问要不要重新下载）· 加歌弹窗（搜索走 `view.songs({search})` · 加完不关 · `FlatList` · **固定高度**）。**N4i-1 是纯桌面批**：`resolveSourceUrl` / `recognizeSourceUrl` 从 daemon 路由提取进 `portable/download/source-url.ts`（characterization 先行，9 条原有 + 3 条新补，搬完一个字没改地全绿），**两处有意的行为变化**——短链展开后仍是短链 → 拒绝 · 纯文本被当成 url 存是既有行为、只钉住不改。**`reidentifySource` 不是按钮**（桌面也不是，它在引擎里；N4g 的「重新下载」与「点没有文件的歌」两条路就是它的生产入口）。
