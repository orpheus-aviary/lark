...
- **N7 每账号独立工作区：子计划已出，待开工**（`docs/plans/2026-08-26-phase-b-mobile-n7.md` v2，七批 N7a–N7g / 判据 103–123）。**顺序：N7 → N6e 文档大整理 → 发版**（用户 2026-08-26 把它从 v1.1 提前）。🔑 **蓝本是 owl**（`../owl/packages/core/src/profile/`）：**id = `sha256(serverId + "\n" + userId)` 前 32 hex**（确定性 ⇒ 同一账号永远落回同一份；锚是 **server_id 不是 URL**；宽度一旦落盘就冻结）· 保留 id **`local`**（从没登录过的那个）· **零迁移是明写的设计**（老库原地当 local，账号库在 `libraries/<id>/`）· **并入 = 整库 COPY 绝不 move**（同步永不写 local 库）· 桌面多一整块 **switch-lock**（原子写 + owner nonce + pid 存活 + 30s TTL；多进程才需要，手机不需要）。**用户 2026-08-26 定的五条**：桌面也一起做（含设备列表过滤）· **手机不做迁移**（旧数据不管，反正能重装重拉；**只有桌面做一次简单迁移**——已绑定的 nest 库按 `sync_binding` 算出 id 后移进 `libraries/<id>/`）· **切换要确认框**（「切换账号需要重启应用，同意/取消」）· **统一缓存上限口径**（显示当前/其他工作区占用，**优先清其他工作区里没固定的**）· 🔴 **设置里的中文说明要跟着改**（判据 **118**：N6c 那句「一个曲库只能绑一个账号…只能清除应用数据重来」改完就是假话）。**三条去风险事实**：**schema 不动**（工作区是库外面的一层，没有 v4）· 七个设备级读取器在桌面的调用处**各为 0**（已实测 ⇒ N7a 对桌面零影响）· **跨工作区清理只删文件不写库**（`has_file` 是探盘得来的，别的库只读打开即可）。🔴 **判据 122：本批大改桌面 ⇒ accept 全系列 126 条不可省**（顺带兑现 N1 判据 22 的旧账）。
