...
🚀 **v0.3.0 已发布（2026-08-17）——m4a 统一 + 一次性迁移 + PC 三项**。[Release](https://github.com/orpheus-aviary/lark/releases/tag/v0.3.0)（`Lark-0.3.0-arm64.dmg`，sha256 `0dfd78c0…`）+ [`@orpheus-aviary/lark-cli@0.3.0`](https://www.npmjs.com/package/@orpheus-aviary/lark-cli)，tag → `9cf9d97`。主计划 `docs/plans/2026-08-13-m4a-and-mobile-master-plan.md`（v11）+ 子计划 `docs/plans/2026-08-13-m4a-unification.md`（v3 + **§9 附表 A 错误分型表**，判据 1–61、决策 a–n 全定）。批次 **T0a → T1 → T1b → T2 → T3 → T4 → T5 → T5b → T6 全部完成**；之后是 Phase B（Android，`apps/mobile`）。canonical 是 `songs/<id>/song.m4a`，`/audio` 回 `audio/mp4`；**schema v3**，迁移从 core 一路接到 daemon / GUI / CLI——**一个 0.2.x 曲库能自己走完转换并把窗口交回曲库**（判据 33 真实副本闭环：7/7 done、0 lost、0 blocked）；导入收 **shipped profile 读得开的一切**（m4a/mp4·aac·mp3·flac·wav·ogg/oga/opus），扩展名只当过滤器；PC 三项已交付，**协议定稿在 `LOCAL_API_VERSION = 6`**。发版时测试 **2419**（shared 79 / core 985 / cli 417 / daemon 495 / gui 443）+ e2e 19 + accept 系列全绿。**schema v3 与 mp3→m4a 都是单向的**：0.3 开过的库，0.2.x 不再打开，音频也回不去。
...
🛠 **Phase B（Android，`apps/mobile`）开发中**——主计划 `docs/plans/2026-08-13-m4a-and-mobile-master-plan.md` §4（**§4.3 已于 2026-08-17 Stage-1 修订两处**）+ N0 子计划 `docs/plans/2026-08-17-phase-b-mobile-n0.md`（v4，判据 1–26 / 决策 a–l / R1–R5）。批次 N0a → N0b → N1 → …；**N0b 全部完成，GO**（N0a + N0b-1/2/3/4a/4b/4c/5a/5b）（`@lark/core/portable` + DatabaseContract 57 例 + `spikes/mobile-foundation/` 真机跑通契约 / bootstrap / drizzle 定案 / 卡顿·crypto·全局面 / bilibili 与 skybridge 探针 / 播放判定 / 分享 intent / D16 机制，桌面测试 2481）。**D4 出口已冻结**：expo-sqlite 57.0.1 + per-call transient shim + drizzle 走 `pnpm patch`（`patches/drizzle-orm@0.38.4.patch`，未打补丁时 10k 查询漏 10000 条语句）。**N0b-3 的 release 实测**：冷启动余量两个数量级、backfill 分段 500 稳过、**apply 的生产批 500 过不去 → 暂定 200/批**（R5 定稿）、md5 端口 0.02ms 而 sha256 的阈值改绑**真实歌词尺寸**（1.94ms；256KB 上限的 86.81ms 降为标注最坏值）、**`globalThis.crypto.getRandomValues` 不存在**、**`atob` ≠ `Buffer.from(base64)`**、**`globalThis.fetch` 就是 `expo/fetch`**（三条行为全过）。**N0b-4 的实测**：判据 22 四条硬 gate + SSE 全绿、判据 23 双网络全绿、**D17 冻结 = raw fMP4 直存达标（GO，不进任何 remux 兜底）**、判据 24 绿（真 bilibili 8.83.0 分享 → 系统 chooser 里有我们 → 冷启动 +15ms 收到原文，`expo-share-intent@8.0.1`）、**判据 26 绿 = D16 机制落定**（copy-then-open：50MB 库 max 75ms、带 4MB 热 WAL max 150ms，原件零写；SecureStore 卸载重装读不出；backup 排除三层 10/10）、**判据 25 绿 = D14 落定**（applicationId `com.orpheusaviary.lark` · 0.1.0/versionCode 1 · keystore 与密码同放 `orpheus-aviary/android-keystore/`，恢复演练过 · adb 安装豁免 developer verification，2027 全球扩大才相关）。**GO/NO-GO = GO**，Stage-2 已把 D4/D16/D17 写回主计划 §4.3。**N1 已于 2026-08-19 全部完成**（子计划 `docs/plans/2026-08-18-phase-b-mobile-n1.md`，九批 N1a–N1i，判据 1–22 + R1–R5）：core 的**整个业务图**进了 `@lark/core/portable`——sync 全图、library 全图、SyncCoordinator、LibraryService（+ 跨前端 LibraryContract）、download client 层与编排；桌面之外只剩这台机器真正拥有的东西（`db/` 的打开与锁、ffmpeg 与落盘协议、file-op 执行器、config、logger、paths 根解析）。Metro 图 36 → **97 个 portable 模块**，bundle smoke 已进 `just check`。**R1–R5 真机全绿**（release · V2408A）→ **D5 分段冻结**，冻结文本见 N1 子计划 **§8.1**（单一事实源；N0 子计划 §3.4 已加 superseded 指针）。**唯一未做的是判据 22**：对新构建的 dmg/tgz 复跑 accept 全系列——按用户决定并入下个桌面版本的发版流程。下一步 **N2**（`apps/mobile` 本体 + 数据层 + mobile contract hook），**开工前另出子计划**。数值判据一律 release 构建。逐批状态见 `PROCESS.md` 的 Phase B 段，判据结果、版本冻结与设备档案见子计划 §9。

**spike 的两条常驻规矩**：① **bundle** 只许 import `@lark/core/portable` / `@lark/shared` / skybridge SDK（第六条守卫 `check-spike-mobile-imports.sh`；第七条是 N1i 收编的 Metro bundle smoke），**禁止复制 core 实现来假装验证 core**——需要 core 算的输入一律由桌面产 fixture；**唯一豁免是 `spikes/mobile-foundation/scripts/*.mjs`**（主机脚本，不在 Metro 图里，产 fixture 时必须用真 core，N0b-4a 起）；② **Expo 已进桌面 workspace，每次 `pnpm install` 变动后必须复跑 `just check` + `just test`**（判据 13）。**短命夹具不进 bundle**：bilibili 流 URL 两小时过期、skybridge 账号每次新建，由 `probe-host.mjs` 的 `/fixtures/network` 现供。
...
- **skybridge 同步**（v0.2，schema v2 起）：实体 `device_id` 只存 skybridge 注册 ID（本地身份在 `local_metadata.device_uuid`，两域不混用）；凭证在**独立文件** `~/orpheus-aviary-nest/lark/skybridge.toml`（0600，不进 `/config` 通道，**backup 每一层都排除**）；`LOCAL_API_VERSION` = **6**（v0.2 引入 `/sync/*` 与 `/conflicts/*` 时是 5，0.3.0 的 PC 三项升到 6——**这一行以 `packages/shared/src/api-paths.ts` 为准**）
