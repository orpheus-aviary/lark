...
桌面 **0.4.2**（2026-08-27；0.4.1 与 0.4.0 同在 2026-08-27 / 26，0.3.0 在 2026-08-17，首发 0.1.0 在 2026-08-10）·
Android **0.1.1**（2026-08-27；首发 0.1.0 在 2026-08-26）。两端版本号各走各的。
...
字节级进度与独立的任务面板。
v0.4.1 加了一个设置：**自动下载下一首**（默认开）——顺序播到一首没有文件的歌会去取回来再播，
而不是跳过它。**协议号随之从 7 变到 8，CLI 要一起升。**
v0.4.2 给多选加了一颗**「下载」**：把选中的歌里缺的音频补下来（已经在本机的不碰），
登录账号后补齐整个曲库不用再一首首点。协议未变。
