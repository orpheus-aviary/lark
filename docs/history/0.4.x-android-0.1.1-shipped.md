# 桌面 0.4.1 / 0.4.2 + Android 0.1.1（2026-08-27 发布）

> 从 `PROCESS.md` 的「本阶段记录」原样搬来（0.5.0 发版时归档）。这一站的两条线：桌面把 0.1.1
> 做过的那半发出去（`playback.auto_download_next`，协议 7 → 8）再补一颗「只补缺的音频」的多选下载；
> 移动端是 **D2「长期使用复盘」的第一份收成**——用户用了一天列回来十一条，其中只有一条是 bug。
>
> 子计划：`docs/plans/2026-08-26-android-0.1.1.md`。
