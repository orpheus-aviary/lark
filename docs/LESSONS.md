...
- 🔴 **验破法时改错了地方，看起来和「判据是空的」一模一样**（0.5.1 P2）：给 `POST /download/parts` 写完六条判据后验破法，改 `packages/core/src` 里的实现、跑 `packages/daemon` 的路由测试——**两条纹丝不动**，第三条（改的是 daemon 自己的源码）正常变红。差一点就下结论说那两条是空判据。真相是 **daemon 把 `@lark/core` 解析到 `dist`**，改 src 根本没进那次运行；而它之所以没有立刻穿帮，是因为 `just typecheck` 的 `tsc -b` 会**顺带 emit**，新加的 `fetchParts` 早已被带进 dist，测试于是正常通过。补上 `just build-core` 之后，四种破法全部按预期变红。**跨包验破法，中间必须重建被改的那个包**；一次「破了也不红」在下结论之前，先证明破法本身进到了运行里（`grep` 改后的产物，或先让它以别的方式红一次）。
- 🔴 **`onClose` 是个 mock，所以测试可以在一个真实应用早已扔掉的对话框里继续点**（0.5.1 PC 上手验收，用户实测）：多行粘贴撞到多 P 时就地展开分P，判据「勾一个再确认 → 发成一个批次」全程绿；而真机上人看到的是「多 P 不会被展开，会失败」。原因是 `confirm()` 忽略 `submitSingles()` 的返回值**无条件 `onClose()`**——分P 确实被列出来了，然后承载它们的对话框在同一个 tick 里关掉。jsdom 里 `onClose` 只是个 `vi.fn()`，组件不卸载，于是「再确认一次」在测试里走得通。**判据断言了内容，没有断言那扇门还开着。** 补的两条一个钉「问了问题就不许关」，一个钉「别的失败照旧要关」——修法走过头同样会红。**凡是「这一步之后界面还在不在」这类事实，要显式断言，不能靠组件恰好还挂着。**
- 🔴 **「盖住别人的全屏应用」和「留住程序坞图标」在 Electron 的默认写法下是互斥的**（0.5.1 A2，用户实测）：`skipTransformProcessType: true` 换回了 Dock 图标，代价是歌词窗**留在原来的 Space**——切到全屏应用（它会开一个新桌面）之后歌词就被盖住了。macOS 要浮在别人的全屏 Space 上，需要窗口的 `fullScreenAuxiliary` **加上「进程不是普通前台应用」**，而后者正是 Electron 那次 `TransformProcessType` 在办的事。**两者都要，就不能靠降级进程去满足第二个条件**——改成 `type: 'panel'`（macOS 非激活面板）：它凭「自己是面板」满足，而不是凭「应用降级」。实测面板下 dock 仍在、进程仍是 `Foreground`、置顶与鼠标事件都正常。**一个平台 API 的「顺手做的事」可能正是另一个需求的唯一实现手段，绕开它之前先问清楚它在替谁干活。**
- 🔴 **`accept-sync` 要的不是「删掉 `libraries/` 的副本」，是「一份从没被认领过的库」**（0.5.1 发版门禁，绕了两轮）：这套件自己调 `backupNest()` 复制 `LARK_NEST_DIR`，然后在上面真的登录一次。而 **`just backup-nest` 复制的是当前活动工作区**——机器主人现在活动的是账号库，所以副本的 `songs.db` 在 `libraries/<hex>/` 下、根目录没有；照旧笔记「删掉 `libraries/` + `workspaces.toml`」等于把唯一那份库删了（`SQLITE_CANTOPEN`）。把账号库提到根同样不行：**那份库带着 device 绑定和 `sync_changes`**，D 组「登录后应该全部待发布」当场对不上，症状和「用户已登录」逐字相同（红三条 + `countSongs` 崩）。**能用的夹具是照 `local` 工作区自己造一份**：`lark_config.toml` + `songs/` + 用 sqlite backup API 复制的根 `songs.db`，不带 `libraries/`、`workspaces.toml`、`skybridge.toml`。**一个「未登录」的库不是「删掉登录痕迹的库」，是从来没登录过的那一个。**

## 两端对齐普查（2026-08-31，发版前用户要求做一次）

- 🔴 **写进 `INVARIANTS.md` 的「两端一致」不是一致，是一句话**（分P 那条）：0.5.1 当天写下「桌面和手机都把多 P 渲染成同一种组……**提交时新建歌单**」，三天后逐功能对照才发现**手机压根没建歌单**——它提交到「存到」选的那个目标，标题只读。两边各写了一份 wire shape，谁也没红。**一条约定不会失败，一个两个调用方共用的函数会**：改法是把那段提进 `portable/download/batch-groups.ts`，桌面的 `groupPayload` 委托过去，并在桌面的测试里直接断言 `groupPayload(parts)` 与 `partsGroupPayload(...)` 逐字段相等——**谁再写第二份就红**。规矩：凡是「两端一致」的规则，落点必须是一处代码。
- 🔴 **判定留在只有真机能加载的文件里，等于没有判据**（同上，根因）：手机那半的目标是在 `ui/parts-picker.tsx` 里拼的，而 `apps/mobile/vitest.config.ts` 的白名单挡住一切 import react-native 的文件 ⇒ 那一行**从来没有被任何测试看过**。桌面那半在 `batch-groups.ts` 里、有测试，所以只有手机走散。这正是 `INVARIANTS.md` §7 最后一条（「wiring 归装配根，判定留在能加载的文件里」）——**它这次是以「两端不一致」的形式收的费**。
