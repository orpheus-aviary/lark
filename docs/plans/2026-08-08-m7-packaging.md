...
- 判据（打包产物最外圈不透明像素）：修前 39/256 全灰 → 第一次修仍有残留 → 现在 **0/128**，与 owl 一致。
  - ⚠️ **这条判据在 0.2.1 被推翻**（2026-08-13）：它只防得住源图的灰光晕，防不住 macOS 给「alpha 不像一块 tile」的图标垫默认灰底板——0.2.0 就是带着那圈灰发出去的。新做法与新判据见 `PROCESS.md` 的 v0.2.1 段与 `build-icons.mjs` 的注释；本节保留为当时的记录。
