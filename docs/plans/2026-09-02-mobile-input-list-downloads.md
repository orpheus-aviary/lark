...
> ⚠️ **本节的输入法做法（每屏包 `KeyboardAvoidingView`）已被上机推翻**，最终实现是「app 根上一处让位」+「sheet 不再用 `Modal`」。原文保留是因为它记着当时的推理和那次读数的用途；**以 §9 为准**。全选那半（非受控 + 一次性 `setSelection`）如实生效。
...
### 2026-09-02 · 第二次上机的结果，以及 `Modal` 的退场

第二次装机：**根上让位成了**（设置页字段、添加页「下载」按钮都在键盘上方）· 歌单高亮 · 新顶栏 · 队列滚动条 —— 全绿。三处新问题：

1. **迷你条和 tab 栏跟着被顶上去**。这是「把 `adjustResize` 还回来」附带的那一半，没人想要。→ **键盘弹起时这两条整个隐藏**，顺带把 ~112dp 还给表单。
2. **弹框顶过头（下面一块空黑），而且仍然不自动弹输入法**。
3. **播放队列弹出时整体下移一截**，用户判断「像是下面导航栏消失导致的」。

**②③ 查下来是同一类东西：第二个窗口。**

- 弹框是 `Modal`，也就是**另一个窗口**：根上那份让位到不了它；而 IME 出现的那一瞬间平台会**平移**这个窗口去露出焦点框，平移量 **JS 看不见**（Fabric 的 `measureInWindow` 算的是布局树，不含窗口滚动）。所以「量出来再减掉」这条路不存在。
- `autoFocus` 失效是同一个窗口的另一头：它在 `onAttachedToWindow` 里跑，而 RN 是 `dialog.show()` 的**下一行**才清 `FLAG_NOT_FOCUSABLE`。换成 `onShow → focus()` 也没成，因为 `requestFocusProgrammatically` 还要 `isInTouchMode`，那仍然是在赌新窗口的时序。

→ **决定（用户 2026-09-02）：sheet 不再用 `Modal`，改成 app 自己窗口里的浮层**（`ui/overlay.tsx`）。浮层绝对定位在**根的 padding box 内**，而根的 padding 就是键盘让位——**弹框在键盘上方，是因为整个 app 都在**，不需要第二个数、也没有要修正的量。`autoFocus` 同理恢复。

**这撤销了 N2f 的一个记录在案的决定**（「sheet 用 RN 自带的 Modal，省三个原生依赖」）。它当时的理由（不引手势库）仍然成立，变的是 Modal 自己带来的第二套几何。代价两条，都已付：
- **返回键要自己答** —— `ui/back.ts` 加了 `BACK.sheet = 40`（比选择态内层）；两个叠起来的 sheet 靠注册序，新的赢。
- **RN 没有 `createPortal`** —— 根上一个 host + 每个 sheet 一个 slot。**副作用要知道**：sheet 的 children 在 host 的位置渲染，所以它们看到的 context 是 host 的（在 `LibraryProvider` 之内，本仓唯一的 context，所以现在没有影响）。

**范围**：只搬 `sheet.tsx`（改歌名/改歌手/新建歌单/加歌/排序/删除确认）。**picker / edit-link / import-playlist 三个全屏对话框仍是 `Modal`**——它们这轮还没验（清单 L），下一轮看了再决定。

③ 用一行验证用户的判断：给队列那个 `Modal` 加 `statusBarTranslucent` + `navigationBarTranslucent`，让它**从第一帧就是 edge-to-edge**。不加时 RN 让它的 decor 贴合系统栏、于是内容区先预留了导航栏，随后的 insets 又把预留拿走——底部对齐的内容因此**正好下移一个导航栏**。如果这一行没修好，说明判断错了，队列框也该搬进浮层。

### 2026-09-02 · 第三次上机：全绿，这一批到此结束

浮层（改歌名/改歌手/新建歌单/排序/删除确认/加歌）**自动弹输入法 · 贴着键盘上沿、下面没有空黑 · 保存取消都点得到 · 返回键能关**；迷你条与 tab 栏打字时隐藏、收键盘回来；**播放队列的下移消失**——用户「像是导航栏消失导致的」那句判断是对的，两个 flag 就够；上一轮没验的三个全屏对话框（更改链接 / 分P / 导入歌单）**这轮也验了，键盘弹起时底部按钮都在** ⇒ **它们保持 `Modal`，不搬**。

收尾：临时的 `KeyboardProbe` 已删（它的两个问题都答完了——`screenY` 会动、几何是配平的）。

**装包踩到一条**：手机锁屏时 `adb install` 报 `INSTALL_FAILED_ABORTED: User rejected permissions`，看着像权限、其实只是屏幕黑着（`dumpsys power | grep mWakefulness`）。已记进 `LESSONS`。

### ~~下一次上机要看的~~（三次上机后全部走完，保留原文）

1. ~~设置页字段 / 添加页「下载」按钮~~ ✅ **第二次已验绿**
2. **改歌名/改歌手/新建歌单**（现在是浮层不是 Modal）：打开就自动弹输入法 · 卡片贴着键盘上沿、下面**没有空黑块** · 保存/取消都点得到 · **返回键能关掉它** · 三个行为一致
