...
### §8.4 落地修订：判据 20 的回落 ②③ 是同一个守卫（2026-08-20，N2g 实施）

**§2.5 写的四条回落，在代码里只有三个分支**，而这不是实现偷懒，是判据 20 自己的要求逼出来的。

- 判据 20 说「每条都要有反测：把该分支删掉 → 那条必须红」。
- 而 ② `lyrics.length === 0` 与 ③ `currentLrcIndex` 回 `-1` **不是两个能各自杀死的判断**：空数组进 `currentLrcIndex`（`lrc.ts:75`）时 `high = -1`，二分根本不进循环、只能回 `-1`。写成两个 `if`，第二个是不可达的死代码，**删掉它不会有任何用例变红**——那正是判据 20 拒绝的那种分支。
- 落地实现因此是一句 `const line: LrcLine | undefined = lyrics[currentLrcIndex(lyrics, timeSeconds, offsetSeconds)]`，**②③ 共用 `line === undefined` 这一个守卫，各自留一条用例**；删掉守卫，两条一起红。

**这是 P1-4 的同一个形状再走一格**：v1 五条 → v2 四条（「没有歌词」与「纯文本无时间戳」在 `parseLrc` 之后不可区分）→ 落地三个分支（「没有歌词」与「还没到第一行」在 `currentLrcIndex` 之后不可区分）。每一次都是**函数分不开的两件事不该假装分得开**。

| | 状态 |
|---|---|
| §2.5 的四条回落**行为** | **不变**，四种输入各有一条用例，全部回歌名 |
| 判据 20 的「四条 + 每条反测」 | **按三个守卫兑现**：①（mode）· ②③（`line === undefined`，一个守卫两条用例）· ④（`line.text === ''`），加上长度上限的两个变异，共五个变异逐条验红 |
| 判据 21 | **原样兑现**，另加三个变异（读路径写回 / 去掉校验 / upsert 退回 INSERT） |
