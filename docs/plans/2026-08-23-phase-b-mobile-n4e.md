...
### 8.5 上机结果，以及用户 2026-08-23 定下的测试规模（**推翻 §8.3 的排程**）

**一次会话跑完了 24 · 26 · 27 · 28 · 29 · 30①③**，代价是 **6 次构建**，其中 3 次是为了看见一个错误——而设备上真正只有它能回答的只有两件（运行时能力、静默降级的表现）。用户因此定了范围：**测试要简化，优先功能开发**。

**新的默认（从 N4f 起）**：

1. **默认落单测。** 只有设备能回答的才上机：原生模块 · Android 策略（前台服务/权限/备份）· 真网络 · 真机数值。
2. **反测全部搬进单测。** 设备上那套「改代码 → 重建 → 验红 → 还原 → 再重建」取消（每条两次构建）。**本批当场兑现了一次**：多 P 门原本在任何地方都没有直接单测，补上 `portable/download/preflight.test.ts`（10 例）之后，把 `pages.length > 1` 删掉**一秒见红**，而三条「narrowness」用例仍绿——同样的信息，千分之一的成本。
3. **一个里程碑最多上机一次**，跑主路径冒烟；红了再深入。
4. **不做预测性桌面探针**，除非结果本身有歧义。
5. 判据文档照写，但**标注归属**（单测 / 上机 / 不验），不再默认每条都要真机证据。

**代价如实记**：失去设备侧「破了会红」的证据，原生接线的回归会更晚被发现——多半是用的时候，不是跑的时候。这是用确定性换速度。

**本批因此不做的**：判据 30② 的错 key 采样（用户 2026-08-23 决定不跑）· 24 与 30 的设备反测（24 的反测卡在 `add-tab.tsx` 的接线上，RN 组件进不了 Node runner，**记成不做**而不是假装有）· 28 的设备反测（已由上面那 10 例顶替）。
