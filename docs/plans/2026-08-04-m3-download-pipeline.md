...
**开放议题 —— 用户已裁决（2026-08-05）：两项都不做，保持 Go 版行为。** ① ID3 标签与封面：不读也不写，导入仍以文件名作歌名、artist 留空；② av 号：不支持，维持「正则不识别时提示改用 BV 链接」的报错文案。
...
### 7.5 用户验收（2026-08-05，副本 nest + 真实网络）

§4 八项**全过**。②：`source_key` 回写 `BV1Ki4y1y7HC:550103819`、222.5s 真 mp3、`/audio` 200/206/416 头部正确且 206 字节精确、歌词后置任务自动派生、事件序列完整。③：关键词经 DeepSeek 清洗成 `温柔` / `五月天`。④：**无 LLM 时单 P 下载成功且 LLM 端点零调用**；多 P 无 `?p=` 同步 400 且报出真实分P数（15）；加 `?p=3` 成功（2608s、p3 的 cid）；关键词同步 400；歌词走启发式仍有 83 行产出。⑦：伪装成 `.mp3` 的 AAC 被容器检查挡下。⑧：fetch-list 903 + 9 条、batch 建真歌单并回填 id、终态快照写回、1001 条整请求拒且无残歌单。

**验收发现并当场修复的三处缺陷**：

1. **取消/失败的新歌下载留下空目录**。暂存在歌曲目录内进行（同卷 rename 的前提），所以目录先于传输存在；`landSongFile` 只补偿它见过的失败，恢复例程又不把「没有音频的目录」当孤儿。每取消一次积一个。修法：任务未提交且 DB 无该行 → 删目录（这个条件精确排除 redownload / 复用绑定的既有歌）。
2. **import 失败文案暴露内部暂存路径**。ffprobe 报的是它拿到的 `.import.<uuid>.tmp`，用户从未见过该文件；`failed[].path` 本就是正确路径，reason 改为引用它。
3. **同一 stage 被重复上报**。worker 设开场 stage、pipeline 进入时又报一次。改为 stage 未变即不发事件。保留的那次 `resolving` 重复是合法的（绑定 song_id 是真实变更）。

**一处是误判**：953 条的收藏夹只回 903 条，一度被当成静默截断；实测第 48 页 `has_more=false` 自然结束，差额是 bilibili 自己过滤掉的失效条目，原行为正确。顺手加的「撞上页数/条数上限时在 `error` 里说明」仍保留（真上限那条路径原本确实会谎称完整）。

### 7.6 验收后的用户裁决（2026-08-05）

| # | 议题 | 裁决 |
|---|---|---|
| 1 | `resolveSongFile` 是否独立导出 | **不抽**。M5 的「播放无文件歌曲自动下载」加一个 task kind `ensure-file`，复用同一条 `#runDownload`，只改 `needsFile` 判定；不产生第二份 claim + 落盘编排 |
| 2 | 无 scheme 的链接 | **补一次 `https://` 重解析**，仅限固定前缀白名单（`bilibili.com/` `www.` `m.` `space.` `b23.tv/`，尾斜杠是防 `bilibili.com.evil.test` 的关键）；补完照跑全部结构化校验。带 `:` 的输入（端口/凭据）根本到不了修复分支——`new URL()` 会把 `:` 前的部分当 scheme，在 https 检查处即拒 |
| 3 | `download:status` 事件 | **加 `revision`**。`(state, stage)` 本就不唯一（绑定 song_id 时 stage 仍是 `resolving`），不带 revision 则计划写下的去重键无法使用。M4 未动工，此时改代价最小 |
| 4 | fetch-list 上限 | **50 页 → 200 页、1000 条 → 5000 条**。实测 953 条的收藏夹要 48 页，原上限贴着真实曲库 |
| 5 | ID3 / av 号 | **都不做**，保持 Go 版行为与现有报错文案（§6 已同步） |
| 6 | 无 LLM 时歌词选中 `稻香(Demo)` | **不动规则，只记录**。两个候选标题相似度均为 1.0，靠时长惩罚区分而 Demo 版更接近——冻结的启发式在按设计工作。是否给 `Demo`/`伴奏`/`live` 加惩罚等实际使用一段时间后再看 |
