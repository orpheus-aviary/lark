...
3. ~~**邻居没有文件就停下，不是跳过**（`playAt` 的 `has_file === false`）。~~ **⚠️ 2026-08-24 已被 N4g 决策 i 改写**：按有没有手指分——`next`/`prev` 去拿回文件再播，`ended` 跳过没文件的。见 `2026-08-24-phase-b-mobile-n4g.md` §5 决策 i。
...
| **j** | 点一首没有文件的歌 | **可点，明说「下载在 N4 开放」**。吞掉点击的行会被当成坏了（N2f 手测结论）。**⚠️ 2026-08-24 兑现**：N4g 换成真的 ensure-file（点它 = 拿回来再播） |
...
| 8 | 判据 10 含「offset ± 写库并立即生效」，N3c 记作通过 | **全屏页当时根本没有这个控件**——只有模式键 / 上一首 / 播放 / 下一首 / 队列。记录写宽了 | 控件与验收都在 **N3d** 补（0.5s 一步、与桌面同图标；`PROCESS.md` 的 N3c 那条已更正） |
| 9 | 判据 17「设备的调用次数等于主机算出的段数」 | **一半的歌会让这条变成概率题**：开头「作词 / 作曲」半秒一行的段落短于一个 tick，设备漏掉哪一个取决于它的 tick 落在哪 | 主机脚本加 **五个 tick 相位扫描**，数不一致就**拒收这首歌**并指出短段落。真实曲库 7 首里拒收 3 首 |
| 10 | 判据 17 的观测时刻 | `dumpsys media_session` 的 `PlaybackState.position` **只在状态变化时更新**，拿它对时间轴会得出「设备比主机快 3 秒」的假结论 | 次数与播放位置都取自 app 自己的诊断行（**同一帧**）；主机脚本因此收一个「停在第几秒」参数 |
| 11 | 判据 19 的反测「拿掉决策 e 的产物 → 必须复现 N0b-4b 的形态」 | 那个形态**已经量过了**——同一台机器、同一个 expo-audio 版本，而且是在修法存在之前 | **不再装一个坏包**：一个在修复前取得的失败测量就是反测 |
