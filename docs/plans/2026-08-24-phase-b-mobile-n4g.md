...
- **日期**：2026-08-24（**v3 · 已完成**，判据 34–40 + 49–52 全绿，三批 N4g-1/-2/-3，head `79f0f62`+）。**决策 a–j 全关**（见 §5；i / j 是真机会话上长出来的）；§8 是实施修订。N4 全期子计划 `docs/plans/2026-08-20-phase-b-mobile-n4.md` 里 N4g 那一行的展开，判据 **34–40 原样继承**（**34 是 gate**），另加用户 2026-08-24 追加的**「重新下载」**。
...
## §7 本批要结清的账（N4 全期的收尾）—— **2026-08-24 结清**

- ✅ **N3 判据 15 的两条**：「点没有文件的歌」由判据 34 顺带验完（三种歌词各放一次，UI 不炸）· `NO_PLAYER_CACHE_OPTIONS` 已经不在生产面上（§8.2）。
- ⚠️ **判据 11 的「不做启动清扫」反测仍然没有运行时开关**——按 N4e §8.5 的测试规模，**如实记成「有代码、无反测」**，不为它补 acceptance 开关（那要在生产路径上开一个只为验收存在的口子）。**接回条件**：启动清扫再改一次的时候，连同它的反测一起做。
- ⚠️ **N4 全期仍欠的三条，本批没有动，如实留着**：判据 18（dataSync 的 6 小时配额，无真机证据）· 判据 32 的设备半边（部分成功那一行没在屏幕上见过）· 判据 31 按标题而非 bvid 比对。
- 🔴 **本批新造的欠账只有一条**：锁屏 / 通知栏的暂停键不经过 JS，因此**不作废等待中的 ensure**（决策 j 下面已记）。要堵得给 `modules/lark-audio` 加一个 media-session 回调面——**N4i 或之后**，不进 N4g。
