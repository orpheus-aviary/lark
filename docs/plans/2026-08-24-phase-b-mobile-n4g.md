...
**做**：ensure-file（点一首没有文件的歌 → 拿回来 → 从头播，含**最新意图胜出**与**队列快照**，§2.9 已冻结）· **重新下载**（用户 2026-08-24 追加：与 ensure-file 同一行的另一件事）· 缓存管理（真的 `CacheOptions`、用量与限额、手动清理、fail-closed）· 歌单导出到系统分享面板 · **文档收尾**（N4 全期的账在 §7 结清）。
