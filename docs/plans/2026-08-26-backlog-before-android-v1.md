...
| **C16 · 歌单详情自己画行** | ✅ **已完成 2026-09-02**——改成复用 `SongRow`。一次补回：当前曲高亮（以前**完全没有**）· 时长 · 固定标记 · 那张表的滚动条。行为变化一处：点正在播的那首现在是暂停/继续，和歌曲页一致。**C11 / C12 仍在**（选择态脚手架和三份 StyleSheet 还是各一份）。 |
| **手机下载页的空分组也画出来**（桌面 `count===0` 整段不画） | ✅ **有意分叉，2026-09-02 用户决定**。三段（进行中/排队中/已结束）的段名照抄桌面，但**空段照画 + 一行小字**：桌面那是对话框里的一段，空着只是噪音；手机上这一页就是整个屏幕，「没有排队的任务」是有人特地进来看的答案。**下次两端普查别当成漂移。** |
