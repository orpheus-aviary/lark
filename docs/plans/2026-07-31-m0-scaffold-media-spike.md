...
**补充实测（用户验收时连续拖动进度条 200+ 次）**：限速下这个上界会变成硬墙——每条 `bytes=N-` 响应负责「从 N 到文件尾」，60+MB / 256KB/s ≈ 4 分钟才写完，6 条连接被长响应占死后，后续 seek **一个请求都发不出去**，播放 stall 在最后位置（用户日志里 `[main] bytes=…` 恰好 6 条，之后只剩 `seeking` 和 `stalled`/`waiting`）。

- 归因：仍是 ①限速放大。**对照实验**：同样的连续拖动，`--no-throttle` 下 `readyState=4`、未暂停、进度继续推进，完全不卡。生产 `/audio` 直读本地盘，等价于无限速这一侧。
- **试过但不采纳的缓解**：把 206 按 4MB 封顶（返回少于请求的字节数是合法 206）。无限速下工作正常；但在限速 + 连续拖动下媒体元素直接进 `MEDIA_ERR_NETWORK`（比 stall 更糟），故撤回，spike 维持开放式 range。**结论：封顶不是安全的通用解法**，将来如果 `/audio` 面对慢速来源（远端/移动端）需要重新设计，不能照搬这个思路。
- 演示时的正确动作：判据 2 是**单次**拖到未缓冲远端（spike 的「seek 到 90%」按钮），连续拖动打满连接预算属于限速演示的已知边界，不是协议层结论。
