...
| **N5f** ✅ | **一次打包、一次真机会话**：判据 69–73 全绿；**76 跑不了**（要 ~2000 行合成负载） | 设备，用户跑 |
...
| 69 ✅ | 明文开关**关着** → 被挡住（用户实测）；存储那一半 N5b 已关（14 条单测 + 反测） | 桌 + 机 |
| 70 ✅ | 明文开关**开着** → 登录成功；桌面侧 `skybridge.toml` 已写（0600） | 机 |
| 71 ✅ | **首次登录跑全量 backfill**：桌面侧 **已推送 104 条 · 未推送 0**；手机的 11 首歌 + 1 个歌单到达桌面 | 机 |
| 72 ✅ | 两个方向都收敛（用户实测）；两端最终都是 **18 首** | 机 |
| 73 ✅ | **歌曲本体不同步**：桌面 **18 个歌曲目录只有 8 个 `song.m4a`**；手机屏幕上三行「需要下载」 | 机 |
...
| 76 🔴 | **`SYNC_PULL_LIMIT_MOBILE` 在竞争条件下复测**（R5 的明账）——**N5f 没跑**：判据要 ~2000 行才有意义，两端加起来只有 18 首。**需要一次合成负载，单独一批**（R5 的 200 仍是空载下界） | 机 |
...
### 8.8 N5f 真机会话（2026-08-25，一次打包一次会话）

**判据 69–73 全绿。** 手机端由用户跑，桌面端的实证由我从副本的库里读出来。

| 判据 | 证据 |
|---|---|
| 69 | 开关关着填 `http://` → 被挡住（用户实测） |
| 70 | 副本 nest 出现 `skybridge.toml`（0600） |
| 71 | 副本库 **已推送变更 104 条 · 未推送 0**；手机的 **11 首歌 + 1 个歌单**到达桌面 |
| 72 | 两个方向都收敛（用户实测）；两端最终都是 **18 首** |
| 73 | 副本 **18 个歌曲目录只有 8 个 `song.m4a`**；手机屏幕上三行「需要下载」 |
| 附带 | **0 冲突 · 0 文件操作 · 0 死信**；设置 tab 旁无小红点，与库里一致 |

- **游标 `pulled_seq 1729 / pushed_seq 1727`**——序号这么大说明那个 workspace 上确实有 v0.2 soak 时期的历史。用户 2026-08-25 明确选择「就用现有账号，都是测试产物」。
- 🔒 **真实 nest 全程未绑定**：`~/orpheus-aviary-nest/lark/` 里没有 `skybridge.toml`，`songs.db` 大小未变。桌面那一半跑在 `just backup-nest ~/lark-n5` 的副本上。⚠️ **那个副本现在是「已绑定」状态**，以后不带 `LARK_NEST_DIR` 直接起 daemon 会开到真库上。
- **安装时踩到一条**：`am start -n com.orpheusaviary.lark/.MainActivity` **起不来**（进程起、Activity 不到前台、logcat 无崩溃），`monkey -c android.intent.category.LAUNCHER` 能——N4d 的分享 intent 插件把 MainActivity 改成 `singleTask` 并加了 filter 之后，不带 category 的显式启动会静默失败。已进 LESSONS。
- **装包前自查发现一处文案瑕疵**：明文开关的说明里写了反引号，在 `<Text>` 里被原样画出来。修掉后重装。

### 8.9 收尾清理（2026-08-25）

- **删掉三个投机的导出**：`resetSyncHubForTests` / `resetSyncContextForTests` / `resetSyncTriggersForTests`——我按「给测试用」写的，**没有任何测试在用**。项目规矩是不做预留抽象。
- 进程与端口：47100 / 8081 均空闲，Gradle daemon 已停，无 `adb reverse` 残留，工作区干净。
- **N5 未了的三笔，都不是待办以外的东西**：判据 **76**（合成负载，单独一批）· 判据 **80 / 81**（冲突页与失败 file-op 的界面已实现，但没有自然触发条件，也没有自动化）· 决策 **b** 的后台同步（**暂时不做**，留在账上）。**2026-08-25 用户定案**：判据 76 与 80 / 81 **先不做，只记录**——`SYNC_PULL_LIMIT_MOBILE` 保持 200（空载下界当默认值用，竞争条件下的表现仍是未证明的假设），冲突页与失败 file-op 的界面保持「实现了但没人看过」的状态；两者都不进 N6 的范围，实际撞上再捡。
