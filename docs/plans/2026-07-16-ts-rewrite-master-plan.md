...
- **排序字段**（M5 后续）：默认 / 歌名 / 歌手 / **时长** / **创建时间**；字段从下拉选、升降序由按钮切（五字段沿用 Go 的点击循环会变成九态）。
- **多选与批量操作**（M5 后续）：复选框列 + Cmd/Shift 点选 + 表头三态（范围 = 当前视图）；批量固定 / 取消固定 / 添加到歌单 / 移出列表 / 删除，右键菜单在选区内同样按批量走。
- **行状态四通道**（M5 后续）：琥珀 = 正在播放 · 左竖条 = 选中 · **蓝色图钉按钮** = 已固定 · `[需要下载]` = 无文件。两个状态色都是独立 token（`--state-active` 琥珀 / `--state-pinned` 蓝）——shadcn 中性色板的 `--primary` 就是正文色，`text-primary` 当高亮用是隐形的。行内按钮常驻显示，不做悬停才现。
...
- **设置页**：LLM 配置（api_key 只写不回显）、主题、字号、缓存上限 + 缓存状态、窗口尺寸、日志、端口只读展示。**显示列不进设置页**（M5-1）：视图态留 localStorage，TopBar 已有勾选，设置页不造第二真源。
