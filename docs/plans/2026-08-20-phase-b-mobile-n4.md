...
**不做（本批）**：同步（N5） · **TLS / 明文 server 的任何工作**（用户 2026-08-20 决定） · 本地**音频**文件导入（D12） · 歌单**导入**（N6） · 歌曲行的多选批量操作（**2026-08-24 修订：从 N6 提前到 N4i**，见 `2026-08-24-phase-b-mobile-n4h.md` 头部；本批的「多选」只在收藏夹/合集的条目勾选里） · 逐词歌词 / Android Auto / widget · 桌面的任何**行为**变化（三处提取一律零行为变化，判据 1 与 4 守着）。
