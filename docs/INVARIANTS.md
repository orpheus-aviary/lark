...
- 🔴 **四个 tab 一直挂着；「挂载」不等于「在屏」**（2026-09-02）。`shell.tsx` 用 `display: none` 切换，不再卸载——为的是回到歌曲页还在原处，也顺手修掉「设置页填一半按 BACK 草稿全没」。**代价是三条以前靠「卸载」白拿的规则，现在必须显式传 `visible`**：① 可见队列 `useVisibleQueue(build, visible)`（否则离开的歌单会继续拥有延迟播放的队列）· ② 返回键 `useBack(active && visible, …)`（选择态优先级高于 tab，看不见的选择会吞掉「回到歌曲」）· ③ 一切从库派生的值走 `useVisibleView(visible)`（隐藏时冻结、露出时补上；设置页那份缓存占用是**每首歌一次 `statSync` × 每个工作区**，它注释里那句「不是热路径」只在页面会被卸载时成立）。**新加任何挂在 tab 上的订阅，都要回答「隐藏时它该不该跑」。**
- 🔴 **键盘让位在 app 根上，一处**（`App.tsx` + `ui/keyboard.ts`，2026-09-02）：targetSdk 35+ 之后 edge-to-edge 强制生效，`adjustResize` 被停用，平台改成**只平移出焦点那一个 view** ⇒ 输入框可用而它下面的按钮被盖住。根上垫 `根高 − screenY` 等于自己实现 `adjustResize`。**别在单个屏幕上包 `KeyboardAvoidingView`**——它拿父容器坐标减屏幕坐标，需要一个推导不出来的 `keyboardVerticalOffset`。键盘弹起时**迷你条和 tab 栏隐藏**（那是让位附带的、没人想要的一半）。
- 🔴 **sheet 不是 `Modal`，是 app 自己窗口里的浮层**（`ui/overlay.tsx`，2026-09-02 撤销 N2f 的那条）。第二个窗口带来第二套几何：根上给键盘让的位到不了它，而 IME 出现时平台会**平移**那个窗口、平移量 JS 读不到，`autoFocus` 也会撞上 dialog 的 `FLAG_NOT_FOCUSABLE`。浮层在根的 padding box 内，所以**它在键盘上方是因为整个 app 都在**。代价两条：**返回键自己注册**（`BACK.sheet`），**children 在 host 的位置渲染**（context 取自 host，它在 `LibraryProvider` 之内）。**picker / edit-link / import-playlist 仍是 `Modal`**，它们没有必须让开键盘的输入。
- 🔴 **`Modal` 是独立窗口，藏起它背后的面板不会关掉它**——每个持有 Modal 的页面在 `visible` 变 false 时要自己关。唯一能触发的是**程序化切 tab**（分享进来会跳到「添加」），因为 Modal 盖住 tab 栏、也自己吃返回键。
...
- **五套桌面 accept 是发版门禁**：`accept-gui`(20) · `accept-cli`(27) · `accept-m5`(22，真 bilibili) · `accept-sync`(36，真 server + 两台 daemon + 真 GUI) · `accept-pack`(29，对新构建的 dmg/tgz) = **134**。**桌面改过就要复跑**；没改过、且能证明「上次跑完只动过文档」时可以复用那次结果（0.5.1 发版就是这么做的，复用前跑 `git diff --name-only` 证明）。
- 🔴 **`accept-sync` 要一份从没登录过的库**——`just backup-nest` 复制的是**当前活动工作区**（这台机器上是账号库，带 device 绑定和同步 outbox）。照 `local` 自己造一份：`lark_config.toml` + `songs/` + **只读打开 + sqlite backup API** 复制的根 `songs.db`，不带 `libraries/` / `workspaces.toml` / `skybridge.toml`；然后 `LARK_NEST_DIR` 指向它。**绝不用 `createDatabase` 碰源库**（§1：那会把它单向升级）。
