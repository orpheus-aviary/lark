...
//      running out is not.
//   2. `shuffle` picks another song, never this one again, and only from the
//      ones with a file.
//   3. A neighbour with no file STOPS the sequence. It is not skipped. (Go did
//      this, the desktop copied it, and the reason is that skipping quietly
//      turns "this song is missing" into "this song does not exist".)
...
export function decideNext(input: QueueDecisionInput): QueueDecision {
  ...
  if (mode === 'shuffle' && trigger !== 'prev') {
    ...
    const pool = songs.filter((song, i) => i !== index && playable(song));
  ...
  if (!playable(target)) {
    // Rule 3, split the same way rule 4 is.
    return trigger === 'ended'
      ? { kind: 'stop', reason: 'no-playable' }
      : { kind: 'reject', reason: 'no-file' };
  }
