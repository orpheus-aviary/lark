...
//
// Three of them live in `portable/errors.ts` instead, because the portable
// modules throw them and portable code may not import core (N0a). They are
// re-exported here so this file stays the one place to look, and so every
// existing consumer — `instanceof` in the daemon, `err.name` in the CLI —
// keeps seeing the same class objects it always did.

export {
  DestructiveForwardMigrationError,
  ForwardMigrationError,
  SchemaMismatchError,
} from './portable/errors.js';
