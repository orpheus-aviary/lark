...
### 下一步：桌面 0.5.0 + Android 0.2.0（子计划已定稿，2026-08-28，尚未动手）

用户按实际使用列回来八条 —— 两条是 bug（输入法组字时回车直接提交 · 手机歌词播到最后不再滚动），其余是产品形状与新功能：解析结果返回重编辑 · 加歌页草稿 · **下载记录带来源 + 桌面补上记录持久化** · **桌面歌词** · 手机歌词中线跳转 · 清洗命名认识多个歌手。

**协议 `LOCAL_API_VERSION` 8 → 9**（`[desktop_lyrics]` 配置段 + `POST /download/batch` 的 `source` + 三个 `/download/history` 端点），**schema 仍是 v3，无 migration**。12 批 / **20 次代码提交**（P1–P10；发版的文档提交另算），真机会话全期只安排一次，排在抬版本号与构建装机之后。
