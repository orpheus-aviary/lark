...
**当前状态（2026-08-17）**：**N0a 全部 + N0b-1 已完成**。桌面测试 **2480**；spike 已在真机跑起来（vivo V2408A / Android 15，档案见子计划 §9），Metro 解析 `@lark/core/portable` / `@lark/shared` / skybridge SDK 全部通过。下一步 **N0b-2**（expo-sqlite shim 跑契约 harness + 迁移链 + drizzle 三选一定案）。
