...
## 当前阶段：**刚发完 Android 0.2.2，没有进行中的批次**

**2026-09-02 发布**：[Release android-v0.2.2](https://github.com/orpheus-aviary/lark/releases/tag/android-v0.2.2)（tag → `17f635b`）。**第一次只发一条版本线**——这一批只动 `apps/mobile/`，所以桌面 `0.5.1` 与 CLI `0.5.1` 留在线上不动。这一站是什么、门禁与三次上机的逐条结果 → **`docs/history/android-0.2.2-shipped.md`**。

| | |
|---|---|
| **线上是哪一版** | 桌面 `0.5.1` · CLI `0.5.1`（npm latest）· **Android `0.2.2`（versionCode 5）** |
| **协议 / schema** | `LOCAL_API_VERSION` **10** · **schema v3**——这一批都没碰 |
| **测试 / 验收** | 3629 · `just check` 绿（十五条守卫）· 真机三次会话走完。**桌面五套 accept 未跑，因为桌面产物没变** |
| **产物哈希** | apk 88,613,828 B `0752f82e…ddbc857c`。**发出去之后回读校验过**：从 Release 下回来的与本地逐字节同哈希 |
...
| **Android 0.2.2**（2026-09-02，**只发手机那条线**：输入法 · sheet 退出 Modal · 列表 · 下载页三段） | `docs/history/android-0.2.2-shipped.md` |
