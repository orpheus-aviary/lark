...
### 下一步：桌面 0.5.0 + Android 0.2.0（**代码与人工验收全过，等发版**，2026-08-28）

用户按实际使用列回来八条 —— 两条是 bug（输入法组字时回车直接提交 · 手机歌词播到最后不再滚动），其余是产品形状与新功能：解析结果返回重编辑 · 加歌页草稿 · **下载记录带来源 + 桌面补上记录持久化** · **桌面歌词** · 手机歌词中线跳转 · 清洗命名认识多个歌手。
...
**现在的位置**：P1–P11 代码提交完毕，`just check` 绿（**十四条守卫**——0.5.0 新增 IME 与「桌面歌词窗禁拖拽区」两条，并补上 `INVARIANTS` §2 一直漏记的 IME 那条），`just test` **3547**（0.4.2 是 3415）。**人工验收全部走完**：桌面 29 + 追加 9 条、真机 14 条 + 1 观察项 + 追加 6 条，**全过**（清单与逐条结果 → `docs/plans/2026-08-28-manual-check.md`）。**没有跑过的**：五套 accept 套件（发版门禁，正在补跑）。

**验收长出来的三批修订**（都在子计划 §7）：

- 🔴 **修订 1 · 判据 19 红**：桌面歌词窗整窗是 `-webkit-app-region: drag`，而拖拽区**吞掉它自己那条控制条赖以出现的 hover** ⇒ 五颗按钮和把手一次都没出现过（右键弹出的 macOS 窗口菜单是同一件事的另一面）。改成 pointer 手动拖 + 把手自己 resize（Electron 文档：透明窗口不可 resize，那个把手本来是句空承诺）+ 悬停画出窗口范围。**这件事没有任何测试看得见**——jsdom 不认识 `-webkit-app-region`，vitest 又把 CSS import 桩成空串，写出来的会是一条空判据——所以落成第十四条守卫。
- **修订 2 · 桌面歌词实时预览**（用户提）：行数/字号/配色不看见就选不出来。预览 = **把草稿 publish 给那个窗口**，一个字节都不写盘，所以「不保存关掉就变回去」不需要撤销这一步；`locked` 与几何不可预览。顺带修掉一个真 bug：`buildPatch` 拿草稿和**当下**配置做 diff，而草稿只在打开那一刻建一次 ⇒ 设置页开着时拖动歌词窗再保存，窗口会弹回去。改成**只发人碰过的字段**；没碰过的字段则反过来**跟随配置**，所以控制条改的东西、拖出来的新位置，设置页里也跟着变。
- **修订 3 · 真机会话长出来的四条**：首页排序本地持久化（`portable/song-sort-pref.ts`，和 `play-mode` 同一套设备设置）· 歌词待跳转行加粗 + 整行可点 · 🔴 **取消一个正在搜歌词的任务从来不会变成「已取消」**（`allSettled` 与「never throws」两个正确决定合起来把 abort 吃干净，按了取消的人被告知网络断了）· 歌单里看不出哪几首还要下载（歌单画的是自己那份 row markup，和 `SongRow` 走散了）。

**下一步是发版**（九步照 `docs/plans/2026-08-08-m7-packaging.md` §3.5，每步用户确认）：五套 accept → 打包 dmg + `npm pack` → `accept-pack` 对固定产物 → tag → Release + npm publish → 安卓 tag 与 Release（APK 已构建并真机验过，**发出去的就是那一份**）。
