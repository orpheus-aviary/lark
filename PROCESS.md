...
| 批 | 状态 |
|---|---|
| **P1 · 输入框** —— `Prompt` 改非受控 + 一次性 `setSelection`（全选那条**已验绿**）· 键盘让位见 P5 | ✅ |
...
| **P5 · 第一次上机之后**（2026-09-02） —— 键盘让位从「每屏 KAV」改成**根上一处**（平台只 pan、不 resize）· 弹框各自垫 + `onShow` 起输入法 · 播放队列表的滚动条 · **歌单页高亮正在播的歌单** · **C16 收掉**（详情复用 `SongRow`）· 歌曲页顶栏两行（✕ · 返回顶部 · 定位当前） | ✅ 代码已落地，**待第二次上机**（计划 §9 末尾八条） |
...
**第一次上机已跑**（2026-09-02）：§6 十二条里**三条红线全绿**（返回键不被隐藏页抢 · 可见队列 · 隐藏页不重算），全选、下载页三段、滚动条也绿；**A/B 红**，而红的方式把修法从「每屏 KAV」换成了「根上一处」（详见计划 §9）。P5 的代码已落地、`just check` 与 `just test` 3629 条全绿，**下一步是第二次上机**（§9 末尾八条），之后删掉临时的 `KeyboardProbe`。这一批之后的候选，按现在看得见的顺序：
