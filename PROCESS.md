...
- **判据 9/10/12 由用户多轮手测通过**，产出 **11 处改动**（下面两段）。**判据 11**（琥珀播放行）截图确认。⚠️ **判据 15 的「点没有文件的歌」没验**——这个夹具库里没有 `has_file === false` 的歌；歌词三态里「无歌词」也没遇上（4 首都有歌词），**如实记着**
...
| N3 | 播放：PlayerDriver + 队列与四模式 + minibar / 全屏页 / 队列面板 + 后台锁屏 + 蓝牙歌词接线（六批 N3a–N3f） | 子计划 `docs/plans/2026-08-20-phase-b-mobile-n3.md` 判据 25 条 | 🔄 **N3a–N3c 完成**（判据 1–15，其中 3②③ / 4 的设备那半随 acceptance、15 的「无文件的歌」夹具里没有）；下一步 **N3d 蓝牙歌词接线** |
