...
| **测试** | 0.5.0 发版时 **3547**（`just test`），`just check` 绿（守卫十四条），五套 accept 对发出去的那份产物 **129/129**。**0.5.1 在做**：守卫 **十五条**（加「桌面歌词窗不许降级整个进程」），`accept-gui` **15 → 18**、五套合计 **129 → 132**。 |
...
1. **程序坞回归 —— 已落地**。修复、第十五条守卫、accept-gui 三条新判据都在。屏幕上那三条（Dock 图标 · 全屏覆盖 · 反复开关）按用户的决定**对打包出来的 0.5.1 验**，还没验。
2. **多 P 选集与分P命名 —— 设计已定稿（§7），尚未实现**。用户决定塑进同一版。P1–P6 分批见 §7.7，**P1 先走**。落地之后协议 9 → **10**、**Android 也要发 0.2.1**，§1–§6 已经跑绿的门禁全部作废要重跑。
