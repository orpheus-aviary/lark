...
**现在的位置**：P1–P10 十七次代码提交完毕，`just check` 绿（十三条守卫，新增 IME 那条），`just test` **3512**（0.4.2 是 3415）。**没有跑过的**：五套 accept 套件、任何真机、任何人眼。

**下一步是验收，不是写代码** —— 先桌面 29 条，再真机 14 条 + 1 观察项 → **`docs/plans/2026-08-28-manual-check.md`**。之后才是 P12 发版（九步照 `docs/history/v0.1.0-shipped.md`）。
