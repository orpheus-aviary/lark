...
### 下一步：桌面 0.5.0 + Android 0.2.0（**P1–P10 全部落地，等上手验收**，2026-08-28）
...
**验收中长出来的第二件事（用户提，已实现待复验）**：设置页改了要点保存才看得见，而行数/字号/配色**不看见就选不出来**。桌面歌词那一节改成**实时预览**——预览是「把草稿 publish 给那个窗口」，**一个字节都不写盘**，所以「不保存关掉就变回去」不需要撤销这一步；`locked` 与几何不可预览（锁上就点不到唯一的解锁开关，几何是窗口自己写的）。顺带修掉一个真 bug：`buildPatch` 拿草稿和**当下**配置做 diff，而草稿只在打开那一刻建一次 ⇒ **设置页开着的时候拖动歌词窗再保存，窗口会弹回去**。改成只发人碰过的字段，保存与预览共用同一个集合；**没碰过的字段则反过来跟随配置**，所以从控制条改的东西、拖出来的新位置，设置页里也跟着变。→ 子计划 §7 修订 2。

**下一步是验收，不是写代码** —— 桌面 §2.5 复验（19、21 + 把手、范围提示）与 §2.6 的 29，再真机 14 条 + 1 观察项 → **`docs/plans/2026-08-28-manual-check.md`**。之后才是 P12 发版（九步照 `docs/history/v0.1.0-shipped.md`）。

分批、判据与逐条决策 → **`docs/plans/2026-08-28-desktop-0.5.0-android-0.2.0.md`**。
