...
- **N4 全期至此**：N4a–N4h 全部完成，**下一步 N4i**（歌曲页多选批量 + 行菜单补齐 + `reidentifySource` 的生产入口）。仍然欠着的三条如实留着：判据 18（6 小时配额无真机证据）· 判据 32 的设备半边 · 判据 31 按标题而非 bvid 比对。
...
- [ ] **锁屏 / 通知栏的暂停键接进 JS**（N4g 决策 j 的缺口）：`modules/lark-audio` 加一个 media-session 回调面，让它也能作废等待中的 ensure。N4i 或之后
