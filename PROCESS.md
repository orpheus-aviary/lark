...
- **N4d 未结的一条**：**判据 24 推到 N4e**（无模型的构建上只有一个可选命名模式，没有可观测差别；17 条单测守着逻辑半边）。**下一步 N4e**：LLM 设置页与它解锁的四条能力（关键词 / clean 命名 / 多 P 选集 / 重新识别），外加判据 24 的设备半边。子计划已出 —— `docs/plans/2026-08-23-phase-b-mobile-n4e.md`（**v1 待评审**，三批 N4e-1–3 / 判据 24 + 26–30 / 决策 a–i 待关闭）。用户 2026-08-23 已拍板两条：**手机上填与桌面同一份 url + model + key** · **设置页加「测试连接」**。
