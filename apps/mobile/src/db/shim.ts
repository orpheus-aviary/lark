// `SqliteLike` over expo-sqlite (N0b-2, graduated from the spike in N2b).
//
// A COPY of `spikes/mobile-foundation/src/sqlite/shim.ts`, and decision d says
// so on purpose: the spike keeps its own because it keeps its own platform
// probes, and two copies will drift. What stops the drift from mattering is
// that neither copy is the truth — DatabaseContract is, and both run it. A
// change here without the same change there is caught by the contract, not by
// somebody noticing a diff.
//
...
// What it costs was measured before this was allowed to be the design (N0b-3
// jank, R5), not assumed.
...
export interface ShimOptions {
   ...
   * A suite whose hard judgement has never been seen to fail is a suite nobody
   * has tested, so the contract gets a deliberately leaky adapter to fail
   * against. Nothing outside the acceptance entry point ever sets it.
   */
  leakOnError?: boolean;
}
 ...
 * core fails on Android — and `sync/rebase.ts` is full of them.
 *
