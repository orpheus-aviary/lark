...
// confirm. Three native dependencies would have been three native
// dependencies.
//
// 🔴 NOT `Modal` ANY MORE (2026-09-02, and N2f's decision is what changed).
// A Modal is a second window, and a second window is why a prompt could not
// be lifted correctly over the keyboard and would not raise it by itself —
// `ui/overlay.tsx` has the whole story. Two things came with the move and are
// easy to miss: the back key is ours to answer now (`useBack`), and these
// children render at the host rather than where they are written.
//
    ...
    <Pressable style={styles.backdrop} onPress={onClose} accessibilityRole="button">
      <Pressable style={styles.card} onPress={() => undefined}>
        ...
        <TextInput
          ...
          // 🔴 `autoFocus` WORKS AGAIN NOW, and it did not while this was a
          // `Modal`: it is applied in `onAttachedToWindow`, one line before RN
          // clears the dialog window's FLAG_NOT_FOCUSABLE, so the keyboard it
          // asked for never came up. In the app's own window there is no such
          // race — the window has been focusable since launch.
          autoFocus
          onFocus={() => {
